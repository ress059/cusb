/**
 * @file
 * @brief Allows code using ECU assertions to be tested. Code should not 
 * continue running under an assert condition as behavior will now be 
 * undefined. To combat this, these stubs throw an @ref AssertException, 
 * which allows control to jump back to the unit test if an assert fires. 
 * Example assuming CppUTest framework is used:
 * @code{.cpp}
 * TEST_GROUP(Group)
 * {
 *      void setup() override
 ...
 * be called at the very beginning of the test. User can also set a custom
